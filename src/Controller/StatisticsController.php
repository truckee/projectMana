...
use App\Form\ReportCriteriaType;
use App\Services\CountyStatistics;
use App\Services\CriteriaBuilder;
use App\Services\Crosstab;
use App\Services\DetailsReport as Detail;
use App\Services\GeneralStatisticsReport as General;
...
//                'excel' => 'Details',
    ...
    public function employmentProfileAction(Request $request, CriteriaBuilder $builder, Crosstab $crosstab)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $em = $this->getDoctrine()->getManager();
            $rowLabels = $em->getRepository('App:Work')->rowLabels($criteria);
            $colLabels = $crosstab->colLabels($criteria);
            $rawData = $em->getRepository('App:Work')->crossTabData($criteria);
    ...
    public function otherServicesAction()
    {
        ...
        $receivingServices = $em->getRepository('App:Household')->receivingServices();
        $receiving = [];
    ...
    private function benefit($criteria, $crosstab)
    {
        ...
        $rowLabels = ['Yes', 'No'];
        $colLabels = $crosstab->colLabels($criteria);
        $data = $em->getRepository('App:Benefit')->crossTabData($criteria);
