 ...
 */
// src\App\Controller\HouseholdController.php
...
class HouseholdController extends AbstractController
{
     ...
     */
    public function newAction(Request $request, Searches $searches)
    {
        $session = $request->getSession();
        $em = $this->getDoctrine()->getManager();
...
//                $searches = $this->get('mana.searches');
     ...
     */
    public function editAction(Request $request, Searches $searches, Addresses $addresses, $id)
    {
        $em = $this->getDoctrine()->getManager();
        $household = $em->getRepository('App:Household')->find($id);
        if (!$household) {
...
//        $addresses = $this->get('mana.addresses');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ('1' === $hasPhysical) {
                ...
                $type = $em->getRepository('App:AddressType')->findOneBy(['addresstype' => 'Physical']);
            ...
            if ('1' === $hasMailing) {
                ...
                $type = $em->getRepository('App:AddressType')->findOneBy(['addresstype' => 'Mailing']);
                $address->setAddressType($type);
                $household->addAddress($address);
            }
     ...
     */
    public function searchAction(Request $request, Searches $searches)
    {
...
//            $searches = $this->get('mana.searches');
     ...
     */
    public function annualTurkey(EnvService $pdf)
    {
        $em = $this->getDoctrine()->getManager();
        $turkeys = $em->getRepository('App:Household')->annualTurkey();
        $year = date('Y');
    ...
    private function initializeHousehold($household, $head, $session)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $relation = $em->getRepository('App:Relationship')->findOneBy(['relation' => 'Self']);
